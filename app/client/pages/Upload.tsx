...
import type { CategoryKey, OCRResponse } from "@shared/api";
...
export default function UploadPage() {
  ...
  const recordTransaction = (data: OCRResponse, noteSuffix: string) => {
    const transaction = {
      id: generateTransactionId(),
        amount: data.amount,
        category: data.category as CategoryKey,
        description: `${data.description || "Receipt"} (${noteSuffix})`,
      date: data.date || new Date().toISOString(),
      createdAt: new Date().toISOString(),
    };
    userDataHook.addTransaction(transaction);
  };
  ...
  async function onUpload(files: FileList | null) {
    ...
    try {
      ...
      if (isImage) {
        ...
        reader.onload = async (e) => {
          ...
          try {
            ...
            recordTransaction(data, "text upload");
          } catch (e) {
      ...
      try {
        ...
        const data = await resp.json();
        toast.success(`Parsed: ₹${data.amount} • ${data.category}`);
        recordTransaction(data, "receipt upload");
      } catch (e) {
  ...
  async function processWithAI() {
    ...
    try {
      ...
      const data = await resp.json();
      toast.success(`Parsed: ₹${data.amount} • ${data.category}`);
      recordTransaction(data, "AI upload");
    } catch (e) {
