...
const CATEGORY_KEYWORDS: Record<string, string[]> = {
  ...
  Shopping: ["amazon", "flipkart", "myntra", "ajio", "store", "shop", "mall", "retail", "clothing", "apparel", "t-shirt", "t shirt", "henley", "jeans", "zara", "shoes", "fashion"],
  ...
  Health: ["pharmacy", "med", "doctor", "clinic", "hospital", "health", "drug", "medicine", "dental"],
};
...
export const handleOCR: RequestHandler = (req, res) => {
  ...
  // Strategy 1: Look for direct labels like "Payment Amount" or "Total"
  let amount = 0;

  const totalKeywordRegex = /(?:grand\s+total|total\s+(?:amount|due)?|payable|amount\s+due|balance\s+due|net\s+(?:amount|total)|final\s+amount|payment\s+amount)\s*[:\s]*(?:₹|rs\.?|inr)?\s*([0-9,]+\.?[0-9]*)/gi;
  ...
  // Strategy 1b: Check for explicit payment amount lines
  if (!amount) {
    const paymentAmountRegex = /payment\s+amount\s*[:\-]?\s*(?:₹|rs\.?|inr)?\s*([0-9,]+\.?[0-9]*)/i;
    const specialMatch = base.match(paymentAmountRegex);
    if (specialMatch) {
      const specialVal = parseFloat(specialMatch[1].replace(/,/g, ""));
      if (isFinite(specialVal)) {
        amount = specialVal;
      }
    }
  }
