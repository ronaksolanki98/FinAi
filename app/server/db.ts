...
export async function initializeDatabase() {
  ...
  try {
    ...
    await client.query(`
      DO $$
      BEGIN
        -- Switch users.id to VARCHAR if it was originally an integer
        IF EXISTS (
        ...
        -- Ensure provider column exists (used for future auth flavors)
        IF EXISTS (
          SELECT 1 FROM information_schema.columns
          WHERE table_name='users'
        ) THEN
          IF NOT EXISTS (
            ...
            ALTER TABLE users
            ADD COLUMN provider VARCHAR(32) NOT NULL DEFAULT 'gmail-otp';
          END IF;
          ...
          -- Clean up legacy authentication artifacts
          ALTER TABLE users
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS users (
        ...
        provider VARCHAR(32) NOT NULL DEFAULT 'gmail-otp',
    ...
    `);
    console.log("[DB] ✓ OTP codes table ready");
    ...
    `);
    console.log("[DB] ✓ Auth sessions table ready");
